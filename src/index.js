...
const { serveRenderFilesMiddleware, e404Middleware, serveAssetsFilesMiddleware } = require('./middlewares')
const { FileSystemManager } = require('./pages')
const { saferesolve, normalizePath, relativeUrl, joinUrl, mergeUrl } = require('./util')
...
function main(options) {
    ...
    const assetsRelativeUrl = mergeUrl(options.assetsUrl)
    ...
    const assetsPages = new FileSystemManager(webrootAssetsPath)
    const adminPages = new FileSystemManager(webrootAdminPath)

    app.use(adminUrl, serveRenderFilesMiddleware(adminPages))
    app.use(baseUrl, serveRenderFilesMiddleware(mainPages))
    app.use(baseUrl, serveAssetsFilesMiddleware(assetsRelativeUrl, assetsPages))
